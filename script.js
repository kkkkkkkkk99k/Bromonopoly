...
function openChest(winnerIdx){ const winner=players[winnerIdx], loser=players[1-winnerIdx], P=currentPools(); chestContent.innerHTML=''; let rewards=[]; if(loser.key==='bro'){ for(let i=0;i<3;i++){ const tool=randPick(P.broTools); const part=randPick(P.sisParts); rewards.push(`${winner.name} 用「${tool}」打 ${loser.name} ${counts}`); } }else{ for(let i=0;i<3;i++){ const act=randPick(P.sisActs); const part=randPick(P.sisParts); rewards.push(`${winner.name} 讓 ${loser.name} 對「${part}」進行「${act}」`); } } rewards.forEach(r=>{ const d=document.createElement('div'); d.textContent=r; d.className='tag'; chestContent.appendChild(d); }); chestModal.style.display='flex'; log(`${winner.name} 獲勝！寶箱開啟。`);}
